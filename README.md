...
Paginated nouns (`releases`, `versions`) support `--limit` (default: 5) and `--page`; `releases --role` filters client-side and continues via the printed `--after` cursor instead. Versions also accept `--country`, `--format`, and `--label` filters. Every navigable name in `artist` and `label` output carries a ref: members and former members, parent label, sub-labels. Use `--json` for JSON output.
